...
目前已完成計算模組並接上寫入時預產流程。有效改寫會在輸出驗證通過後評估；只有 `score > 0.7` 且不等同 1 的改寫會被採用並寫入 `renditions`。評估失敗或 `score <= 0.7` 時不寫入該語氣改寫；門檻只擋離題，人稱、覆蓋等忠實度問題由 `validateRenditionText` 的確定性檢查負責；`score` 等同 1 時視為沒有實質改寫，讀取時顯示原文且不標示為改寫。已建立本地繁體中文校準樣本與排序測試，真實 NIM embedding 金鑰實測仍需持續校準，不可把舊 `scale` 字串直接當成 embedding 分數。

...
| 3 | 動態牆與個人頁每 3 秒背景輪詢第一頁，頂部「載入新貼文」按鈕與返回頁面時也會重抓；新貼文插到最上方且不打掉已捲到的頁，既有貼文就地更新讚數與留言數，背景輪詢不顯示載入狀態 | 別人剛發的貼文 3 秒內出現在最上方，不需重新整理 |
| 4 | 貼文卡：頭像（username 首字＋固定色）、顯示名稱、username、相對時間、內容、改寫幅度分級標籤、讚數、留言數 | 所有欄位齊備，點卡片進入貼文頁；貼文頁的貼文另顯示語意相似度百分比 |
| 4a | 有留言的貼文卡在底下串接第一則留言的預覽（最早的一則，與留言相同的改寫規則），貼文頭像與預覽頭像之間以淺灰直線連接；貼文頁的貼文與留言、留言與留言之間同樣以直線串接，不畫橫向分隔線 | 只預覽一則、不可在卡片上回覆；其餘留言進貼文頁看 |
| 5 | 他人貼文在改寫完成前顯示骨架（作者列＋內容佔位），不顯示原文 | 讀者不會先看到原文再跳成改寫版 |
| 6 | 自己的貼文直接顯示原文，不加標籤 | 不觸發改寫 |
...
flowchart TD
  A[動態牆] --> B[列表附帶讀者語氣的改寫]
  B -- 有實質改寫 --> D[顯示改寫版，標幅度分級]
  B -- 100% 相似 --> E
  B -- 預產中 --> C[顯示骨架，輪詢改寫]
  C -- 撈到 --> D
  C -- 逾時／預產失敗 --> E[顯示原文，標「原文」]
  B -- 無改寫 --> E
  D -- 點「顯示原文」 --> F[原文，標「原文」]
  F -- 再點 --> D
  A --> G[發文：純文字 ≤ 500 字]

...
| 3 | 自備金鑰：Anthropic／OpenAI／OpenRouter 的金鑰只存在使用者自己的瀏覽器，平台不保存；介面明示這一點。可選填模型名稱、輸出上限，OpenAI／OpenRouter 另可填 temperature，沒填用伺服器預設 | 資料庫沒有任何金鑰欄位；金鑰隨每次改寫請求送到伺服器轉交供應商、用完即丟；存了金鑰後改寫改走自備金鑰並解鎖自訂語氣指示；換裝置需重填 |

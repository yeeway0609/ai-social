...
AI Social 在作者與讀者之間加一層**語氣層**：貼文、留言、訊息一律以原文存放，每位讀者用自己選的語氣閱讀**他人**的內容。作者寫什麼就存什麼，沒有 AI 潤飾、沒有預覽；讀者可以把激烈的話緩和成自己受得了的樣子，也可以改成客觀中立或清楚簡潔。同一則內容在兩支手機上截然不同，資料庫裡永遠只有一份原文。
...
| # | 功能／需求 | 完成目標 |
|---|---|---|
| 1 | 預建帳號：無註冊功能，帳號（username、顯示名稱、密碼）由團隊直接寫入資料庫 | 介面上不存在註冊入口；資料庫沒有的 username 無法登入 |
| 2 | 登入：username ＋ 密碼。密碼是團隊指定的短碼（例如 8 碼數字），明文存放，demo 結束即關閉服務 | 正確密碼建立 30 天有效的 session；username 不存在與密碼錯誤顯示同一則錯誤訊息 |
...
| # | 功能／需求 | 完成目標 |
|---|---|---|
| 1 | 首次登入強制進入引導設定：從預設語氣清單選一種（沒有「不改寫」可選，原文只能對單則用「顯示原文」看）；完成前不能進入 App | 未完成引導設定的帳號開任何頁面都被導回引導設定 |
| 2 | 設定頁可隨時修改同樣的內容 | 修改後回到動態牆，他人內容以新設定重新改寫 |
| 3 | 改寫模型由平台統一提供（NVIDIA NIM 的 Nemotron），金鑰只放伺服器；使用者不需也不能填自己的金鑰 | 前端沒有任何金鑰輸入；資料庫與瀏覽器儲存都沒有金鑰 |

引導設定要問哪些項目，以語氣改寫引擎 PRD 的語氣設定為準；目前只問語氣相關設定，顯示名稱由團隊建帳號時給定，頭像用 username 首字加固定色。
...
| 3 | 每則他人內容都有小標籤：看到實質改寫版時只標改寫幅度分級（「幾乎原話」「微調」「大幅改寫」），看到原文時標「原文」；自己的內容不加任何標籤 | 標籤不出現「AI 改寫」字樣；改寫失敗、點了「顯示原文」或改寫結果語意相似度等同 100% 時標「原文」；語意相似度百分比只在貼文頁的貼文上顯示，列表與留言、訊息不顯示 |
| 4 | 顯示原文：他人內容可一鍵切回原文，一次性、不改全站設定 | 切回原文不觸發模型呼叫；再點一次回到改寫版 |
| 5 | 改寫尚未產出或預產失敗時不阻擋 | 剛寫入的內容先顯示骨架等預產，之後顯示原文＋「原文」標籤；瀏覽、發文、留言、聊天皆不受影響 |
...
- 語音朗讀：他人內容以讀者語氣朗讀（ElevenLabs），金鑰同樣由伺服器持有。
...
| PRD | 內容 |
|---|---|
| 語氣改寫引擎 PRD（doc-002） | 語氣設定的值域、改寫不變量、改寫服務契約、觸發時機、改寫幅度、模型與金鑰 |

...
| # | 功能／需求 | 完成目標 |
|---|---|---|
| 1 | 首次登入強制進入引導設定：從預設語氣清單選一種（沒有「不改寫」可選，原文只能對單則用「顯示原文」看）；自訂語氣指示要等填了自備金鑰才能使用；完成前不能進入 App | 未完成引導設定的帳號開任何頁面都被導回引導設定 |
| 2 | 設定頁可隨時修改同樣的內容 | 修改後回到動態牆，他人內容以新設定重新改寫 |
| 3 | 自備金鑰：Anthropic／OpenAI／OpenRouter，或任何使用者自己指定端點的 OpenAI 相容服務；金鑰與端點只存在使用者自己的瀏覽器，平台不保存，介面明示這一點。可選填模型名稱、輸出上限，非 Anthropic 另可填 temperature，沒填用伺服器預設；相容端點必填 base URL 與模型名稱 | 資料庫沒有任何金鑰欄位；金鑰隨每次改寫請求送到伺服器轉交供應商、用完即丟；存了金鑰後改寫改走自備金鑰並解鎖自訂語氣指示；換裝置需重填 |
...
| 3 | 每則他人內容都有小標籤「AI 改寫」或「原文」 | 改寫失敗或點了「顯示原文」時標「原文」，其餘標「AI 改寫」並附改寫幅度 |

...
| 2 | 輸出：呈現文字、是否為原文、改寫幅度標籤、語意相似度、金鑰來源（自備／共用池）、錯誤碼（若有） | 呼叫端只靠回傳值就能決定標籤與提示 |
...
| 5 | 自訂指示的改寫只在用戶端暫存，不寫入平台資料庫，也不與純預設語氣共用 | 同一頁面重複顯示可直接使用暫存；重新整理或登出後需要重新生成 |
...
語意相似度以原文與改寫版的 embedding cosine similarity 計算，前端顯示為百分比到小數第一位（`XX.X%`）。此百分比只是向量接近程度的顯示格式，不代表事實正確率或原意保留率；評估失敗時不丟棄已成功生成的改寫，而是顯示語意相似度暫不可用。
...
| 2 | 供應商：Anthropic、OpenAI、OpenRouter、local；預設供應商與模型由環境設定決定 | 換供應商不需改呼叫端 |
...
flowchart TD
  ...
  J --> K[呼叫模型，10 秒時限]
  K -- 失敗／逾時 --> Y
  K -- 成功 --> L[計算幅度與語意相似度；預設語氣存入快取，自訂指示只回用戶端] --> H
```

...
| # | 功能／需求 | 完成目標 |
|---|---|---|
| 1 | 語氣值域：一組預設語氣，加上「不改寫」這個特例值 | 讀者選「不改寫」時所有內容都是原文，不呼叫模型 |
| 2 | 自訂語氣指示：讀者可填一段自由文字，疊加在所選預設語氣之上 | 改寫結果同時反映預設語氣與自訂指示 |
| 3 | 語氣設定屬於讀者、全站唯一 | 動態牆、留言、聊天室套用同一組設定 |

預設語氣清單的內容尚待與隊友定案，定案後補進本節。
...
| 3 | 讀者語氣為「不改寫」，或內容作者就是讀者本人，直接回原文 | 這兩種情況不產生模型呼叫 |
...
| # | 功能／需求 | 完成目標 |
|---|---|---|
| 1 | 惰性改寫：讀者端在內容即將進入可視範圍前（提前約 3 則）呼叫；不在發文時預先產生 | 發文成功不等待任何改寫；讀者一般情況下不看到骨架 |
| 2 | 切換語氣設定後重新改寫 | 回到動態牆後所有他人內容以新設定重新呼叫 |
| 3 | 讀者端同時進行中的改寫請求最多 3 個，其餘排隊 | 快速捲動不會瞬間送出十幾個請求 |
| 4 | 現階段不做結果快取，每次呼叫都改寫 | 相同內容同一讀者重看會再呼叫一次模型 |
...
| # | 功能／需求 | 完成目標 |
|---|---|---|
| 1 | 金鑰解析順序：讀者自備金鑰優先，其次團隊共用池，皆無則失敗 | 回傳的金鑰來源與實際使用一致 |
...
```mermaid
flowchart TD
  A[呼叫端：內容 id ＋ 類型] --> B{作者是讀者本人？}
  B -- 是 --> Z[回原文，標「原文」]
  B -- 否 --> C{讀者語氣＝不改寫？}
  C -- 是 --> Z
  C -- 否 --> D{解析金鑰}
  D -- 皆無 --> Y[回原文，標「原文」，錯誤碼：無金鑰]
  D -- 自備／共用池 --> E[組合：預設語氣 ＋ 自訂指示 ＋ 不變量]
  E --> F[呼叫模型，10 秒時限]
  F -- 失敗／逾時 --> Y
  F -- 成功 --> G[計算改寫幅度]
  G --> H[回改寫文字，標「AI 改寫」＋幅度＋金鑰來源]
```

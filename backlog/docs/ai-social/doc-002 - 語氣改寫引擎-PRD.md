...
| 2 | 自訂語氣指示：讀者可填一段自由文字，疊加在所選預設語氣之上；只有已填自備金鑰的讀者能設定與生效 | 沒有自備金鑰時欄位停用、後端拒收；金鑰刪除後自動退回純預設語氣 |
| 3 | 語氣設定屬於讀者、全站唯一 | 動態牆、留言、聊天室套用同一組設定 |

預設語氣清單目前為五個候選加「不改寫」，正式名稱與 prompt 由隊友調整，不寫進本文。
...
| 8 | demo 前可用管理工具一次預產所有既有內容 | 工具重複執行到沒有待處理內容為止 |
| 9 | 內容刪除時連同其改寫一併刪除 | 資料庫不留孤兒改寫 |

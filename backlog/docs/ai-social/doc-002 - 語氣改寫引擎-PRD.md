...
| 5 | 保留人稱與視角：原文用「我」改寫就用「我」、用「你」就用「你」；不得改成第三人稱（他、作者、提問者、用戶、對方），不得用「您」對作者說話。輸出永遠是作者本人在說話，不是轉述、回覆、安慰或建議 | 訊息「我為了你花錢」改寫後付錢的人仍是傳訊者；貼文不會出現「提問者」「用戶表示」 |
| 6 | 改寫原文本身，不描述原文：不得輸出「有重複的聲音」「原文為疑問符號」這類對文字的說明 | 純感嘆的內容不會變成對它的解說 |
| 7 | 覆蓋完整：改寫的句數與段數不少於原文；風格化語氣可以把一句拆成多行，但不可整段刪除 | 三段的貼文七種語氣都還是三段 |

提示詞依內容類型說明「誰在說話、對誰說」：貼文與留言是作者對讀者公開說的話；訊息是傳訊者對讀者說的話，訊息裡的「你」就是讀者本人、「我」是傳訊者。提示詞不附任何語氣範例句，風格只用文字描述（範例句會被模型原字抄進輸出）。

### 3.2.1 輸出驗證

每一版改寫寫入前都要通過下列確定性檢查，全部不呼叫模型；任一項不過該語氣留空，讀者看原文：

| 檢查 | 規則 | 錯誤碼 |
|---|---|---|
| 精確資料 | 原文的數字、金額、網址、標籤、帳號必須原樣出現 | `token_changed` |
| 長度上限 | 不超過原文兩倍（至少允許到全站字數上限） | `output_too_long` |
| 人稱守門 | 原文有「我」而改寫沒有且出現他／她／作者／提問者／用戶／對方等旁白稱呼；原文有「你」而改寫沒有；原文沒有「您」而改寫有 | `perspective_changed` |
| 覆蓋守門 | 改寫句數低於原文的 70%，或字數低於原文的 40%（清楚簡潔放寬到 30%）；字數只算字母與數字 | `content_dropped` |
| 洩漏守門 | 改寫與該語氣設定頁範例句共用五個字以上的連續片段（原文本來就有的不算），或出現「如果 A」「那 B」這類佔位字母句型 | `sample_leaked` |
| 描述守門 | 改寫出現「原文」「這段文字」「符號」「用戶表示」「聲音」等描述性字樣而原文沒有 | `meta_description` |
...
| 9 | 沒有命題可改的短內容不呼叫模型：去掉標點、表情、語氣詞與連續重複字後不足四個字（「欸欸欸」「???」「傻眼」「哈哈哈」），所有語氣直接以原文落地，相似度記為 100% | 讀者立刻看到標「原文」的原句，不會先等骨架，也不會看到捏造的內容；補跑時不再重打模型 |
...
語意相似度以原文與改寫版的 embedding cosine similarity 計算，前端顯示為百分比到小數第一位（`XX.X%`）。此百分比只是向量接近程度的顯示格式，不代表事實正確率或原意保留率；生成結果必須通過輸出驗證，且語意相似度必須高於 70% 才採用；門檻只負責擋明顯離題的輸出，忠實度靠 3.2.1 的確定性檢查。評估失敗、分數不可用或分數不高於 70% 時，該次改寫視為失敗，回傳正式原文並標示未改寫原因。分數等同 100% 時視為沒有實質改寫，回傳正式原文並標示「原文」，不寫入新的預設語氣改寫快取。

### 3.6 模型與金鑰

| # | 功能／需求 | 完成目標 |
|---|---|---|
| 1 | 模型固定為 NVIDIA NIM 的 Nemotron 3.5 Lightning（`nvidia/nemotron-3.5-lightning-30b-a3b`），走 OpenAI 相容端點；模型名稱與 temperature 可由環境設定覆寫，temperature 預設 0.3、top_p 固定 0.9，關閉思考模式 | 換模型只改環境變數；忠實改寫不用高溫抽樣 |
...
flowchart TD
  ...
  S --> T{有可改寫的內容？}
  T -- 否：純感嘆、幾個字 --> U[所有語氣直接存原文，相似度 100%]
  U --> Y
  T -- 是 --> P[背景：一次呼叫模型回所有語氣，25 秒時限]
  P -- 成功 --> Q{逐語氣輸出驗證與語意相似度}
  Q -- 實質改寫且高於 70% --> R[計算幅度，存入 renditions]
  Q -- 100% 相似 --> Y
  Q -- 驗證失敗或不高於 70% --> X
  P -- 失敗 --> X[語氣留空]

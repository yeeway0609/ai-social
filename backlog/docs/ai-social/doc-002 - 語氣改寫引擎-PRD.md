...
語意相似度以原文與改寫版的 embedding cosine similarity 計算，前端顯示為百分比到小數第一位（`XX.X%`）。此百分比只是向量接近程度的顯示格式，不代表事實正確率或原意保留率；生成結果必須通過輸出驗證，且語意相似度必須高於 80% 才採用。評估失敗、分數不可用或分數不高於 80% 時，該次改寫視為失敗，回傳正式原文並標示未改寫原因。分數等同 100% 時視為沒有實質改寫，回傳正式原文並標示「原文」，不寫入新的預設語氣改寫快取。
...
flowchart TD
  ...
  Q -- 實質改寫且高於 80% --> R[計算幅度，存入 renditions]
  Q -- 100% 相似 --> Y
  Q -- 驗證失敗或不高於 80% --> X
  P -- 失敗 --> X[語氣留空]

...
| # | 功能／需求 | 完成目標 |
|---|---|---|
| 1 | 改寫只在寫入時產生：貼文與留言送出後，在背景對每個預設語氣各產一份存入資料庫 | 送出本身不等待改寫；讀者之後直接撈 |
...
| 5 | 單次改寫時限 10 秒；失敗（AI 不可用、供應商錯誤、逾時、輸出驗證不過）該語氣留空 | 一個語氣失敗不影響其他語氣 |
...
flowchart TD
  ...
  S --> P[背景：對每個預設語氣呼叫模型，10 秒時限]
  P -- 成功 --> Q[輸出驗證、計算幅度與語意相似度，存入 renditions]
  P -- 失敗 --> X[該語氣留空]
  A[讀者端：列表查詢] --> B{作者是讀者本人？}
  B -- 是 --> Z[原文，標「原文」]
  B -- 否 --> C[join renditions：內容 × 讀者語氣]
  C -- 命中 --> H[改寫文字，標「AI 改寫」＋幅度＋語意相似度]

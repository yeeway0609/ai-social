/**
 * 預建 demo 帳號與示範內容。沒有註冊功能，加人或改密碼都靠這支重跑（以 handle 為鍵 upsert）。
...
for (const user of seedUsers) {
  ...
  const [row] = await db
    .insert(users)
    .values({ handle, displayName: user.displayName, password: user.password })
    .onConflictDoUpdate({
      target: users.handle,
      set: { displayName: user.displayName, password: user.password }
    })
    .returning({ id: users.id })
  idByHandle.set(handle, row!.id)
}
...
function userId(handle: string) {
  const id = idByHandle.get(handle.toLowerCase())
  if (!id) throw new Error(`seed-content 用到不存在的帳號：${handle}`)
  return id
}
...
for (const [index, post] of seedPosts.entries()) {
  ...
  const authorId = userId(post.handle)
  ...
  const likers = handles.filter(handle => handle !== post.handle.toLowerCase()).sort(() => random() - 0.5).slice(0, likerCount)
  if (likers.length > 0) {
    await db.insert(likes).values(likers.map(handle => ({ postId, userId: userId(handle), createdAt: new Date(createdAt.getTime() + random() * 3_600_000) })))
  ...
  for (const [commentIndex, comment] of (post.comments ?? []).entries()) {
    await db.insert(comments).values({
      postId,
      authorId: userId(comment.handle),

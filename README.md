...
| 防線 | 位置 | 做什麼 |
|---|---|---|
| 系統提示 | `server/utils/ai/prompt.ts` | 不變量寫在最前面：只改語氣不改語意、不摘要、原文每句都要有對應；`<original>` 標記把貼文當資料，擋提示注入。每種語氣附一段範例輸出 |
| 輸出驗證 | `server/utils/ai/outputValidation.ts` | 用正規表達式抽出原文的網址、數字（含單位）、`#標籤`、`@帳號`，改寫裡少一個就整筆作廢；長度超過原文兩倍也作廢 |

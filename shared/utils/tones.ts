...
const TONE_DEFINITIONS = [
  ...
  },
  {
    ...
    sample: '如果會議沒有重點。\n\n那重點。\n\n就是會議沒有重點。',
    instruction: '用黃山料體重述：句子要短，每句盡量像看似深刻、細想又像廢話的金句，句與句之間用空一行的換行留白製造情緒停頓。可以用「如果、那、就是」「不是、而是」「你以為、其實」這類對比句型，句型的空位一律填原文裡的內容，不要輸出任何佔位字母或符號。標點只用句號、逗號與問號，不要使用刪節號「…」或連續的點。必須保留原文所有關鍵事實、數字、人物、立場與結論；不要新增原文沒有的故事、承諾、攻擊或結論，也不要把語意改成雞湯或勵志文。'
  },
  {
    id: 'caige',
    label: '財哥體',
    ...
    instruction: '用財哥體重述：在一行文字間大量插入刪節號「…」切分連續文字，讓每一段連續文字盡量不超過三個中文字。語氣可以口語、直接、有停頓感，但只能切分原文的字句，不得新增原文沒有的詞語、投資、金錢、成功學或建議；必須保留原文所有關鍵事實、數字、人物、立場與結論。標點與數字可以保留原樣，不要為了切段破壞網址、標籤、帳號、金額或百分比。'
  },
  {
